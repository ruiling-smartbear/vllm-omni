...
def unwrap_single_media_stream(value: object) -> object:
    """Unwrap the one-item media list a request's conditioning stream carries.

    ``multi_modal_data[modality]`` holds the request's media, so a single
    reference arrives wrapped in a list.  The path that consumes it expects the
    item itself; a longer list is a caller's own container and is left alone so
    the existing "unsupported type" error still describes it.
    """
...
def unwrap_single_video_stream(value: object) -> object:
    """Like :func:`unwrap_single_media_stream`, for a video reference.

    A decoded video is itself a list or tuple of frames, and
    ``normalize_decoded_video_frames`` requires those frames to be PIL images, so
    a single-item list of an image is a one-frame sequence rather than the
    transport's wrapper.
    """
    if isinstance(value, (list, tuple)) and len(value) == 1:
        inner = value[0]
...
class LancePipeline(BagelPipeline):
    ...
    def _forward_t2v(self, req):
        ...
        # Prefill the same upstream segments every other Lance task prefills:
        # system prompt, then the user instruction, then the assistant header.
        # ``_forward_t2v`` used to hand the bare caption to ``prepare_prompts``,
        # which only wraps it in bos/eos, so the model was conditioned on
    ...
    def _forward_i2v(self, req):
        ...
        # A conditioning stream arrives wrapped in the request's media list;
        # ``_forward_image_edit`` normalizes the same way.
        image_input = unwrap_single_media_stream(image_input)
    ...
    def _forward_video_edit(self, req):
        ...
        # A conditioning stream arrives wrapped in the request's media list, but
        # ``list``/``tuple`` is also how a decoded frame sequence arrives, so only
        # a wrapper around something that is not a frame is unwrapped.
        video_input = unwrap_single_video_stream(video_input)
